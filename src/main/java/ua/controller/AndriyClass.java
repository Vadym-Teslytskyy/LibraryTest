...
public class AndriyClass {
    ...
    public static void getBooksByMainAuthor(String authorLastName) {
        List books = em.createQuery("select b.name from Book b join b.mainAuthor c where c.lastName = ?1").setParameter(1, authorLastName).getResultList();
        System.out.println(books);
        em.close();
    }

    public static void getBooksByCoAuthor(String coAuthorLastName) {
        List books = em.createQuery("select b from Book b join b.collaborationAuthors c where c.lastName = ?1").setParameter(1, coAuthorLastName).getResultList();
        System.out.println(books);
        em.close();
    }
